...
int lfs_win32_lstat(const char *path, STAT_STRUCT * buffer)
{
  ...
  //if (GetFileAttributesEx(/*path*/StringFromUTF8(path).c_str(), GetFileExInfoStandard, &win32buffer)) {
...
static int pusherror(lua_State * L, const char *info)
{
  ...
  else
  {
	  ...
	  lua_pushfstring(L, "%s: %s", info, tmp);
  }
...
static int lfs_lock_dir(lua_State * L)
{
  ...
  strcpy_s(ln, pathl, path);
  strcat_s(ln, strlen(lockfile), lockfile);
  //fd = CreateFile(/*ln*/ StringFromUTF8(ln).c_str(), GENERIC_WRITE, 0, NULL, CREATE_ALWAYS,
...
static int lfs_g_setmode(lua_State * L, FILE * f, int arg)
{
  ...
  int op = luaL_checkoption(L, arg, modenames[0], modenames);
